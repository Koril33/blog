...
对于是否进行打分，查询中有两种上下文（这也是关系型数据库没有的概念），一种叫 Filter Context，另一种叫 Query Context。
...
prefix 按照前缀搜索，类似 SQL 里的 LIKE %search，找到名字为 An 开头的学生：
...
wildcard 是 Elasticsearch 里用于模糊匹配字符串的一种查询方式，适合做类似 SQL LIKE 的搜索。
...
1. [https://www.elastic.co/guide/en/elasticsearch/reference/8.19/query-dsl.html](https://www.elastic.co/guide/en/elasticsearch/reference/8.19/query-dsl.html)

...
[https://stackoverflow.com/questions/3297048/403-forbidden-vs-401-unauthorized-http-responses](https://stackoverflow.com/questions/3297048/403-forbidden-vs-401-unauthorized-http-responses)
...
可以借助 [https://www.base64encode.org/](https://www.base64encode.org/) 来看看编码后的结果：
...
[https://korilweb.cn/backend/%E6%8B%A6%E6%88%AA%E5%99%A8%E5%92%8C%E8%BF%87%E6%BB%A4%E5%99%A8/](https://korilweb.cn/backend/%E6%8B%A6%E6%88%AA%E5%99%A8%E5%92%8C%E8%BF%87%E6%BB%A4%E5%99%A8/)
...
### `AuthenticationFilter` 和 `AuthenticationManager`
...
如果验证通过，`AuthenticationManager` 会返回一个 `Authentication` 对象，如果认证失败则会抛出 `AuthenticationException` 异常，如果无法决定则返回 null。

### `AuthenticationProvider` 和 `ProviderManager`

`AuthenticationManager` 最常见的实现类是 `ProviderManager`，`ProviderManager` 将认证逻辑委托给一系列的 `AuthenticationProvider`：
...
`AuthenticationProvider` 有点像 `AuthenticationManager`，不过它多了一个 `supports` 方法，来判定当前的 `AuthenticationProvider` 是否支持认证。换句话说，`AuthenticationProvider` 组成了一个链，由 `ProviderManager` 来代理，认证一个用户可能不仅仅只有 username + password 的方式，也可能通过别的方式来认证，所以 `AuthenticationProvider` 链就很有必要了。

### `UserDetailsManager` 和 `PasswordEncoder`

假设使用的是 username + password 的方式，就需要有获取 `User` 详情的抽象，以及关于密码加密和匹配的抽象，`AuthenticationProvider` 将这部分的责任代理给了 `UserDetailsManager` 和 `PasswordEncoder`。

`UserDetailsManager` 和 `PasswordEncoder` 是相伴而生的，当我们自定义了 `UserDetailsManager` 的实现类之后，我们还必须指定一个 `PasswordEncoder`。
...
重新启动 Spring Boot 程序，发现原来的那个随机的 UUID 密码提示消失了，我们现在需要使用配置文件的 username 和 password 通过验证：
...
本节将重点放在用户相关的接口和实现上面，之前简单提到了 `UserDetailsService`，它主要用于查询用户信息，将查到的信息交给 `AuthenticationProvider`，配合 `PasswordEncoder` 来验证用户身份。它在整个框架中，作为获取用户信息的 DAO 接口。

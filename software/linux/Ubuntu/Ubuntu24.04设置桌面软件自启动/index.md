...
本文主要介绍诸如微信，Clash 之类的桌面软件如何添加到 Ubuntu 的开机自启动中。
...
clash 需要注意启动要加上 `--no-sandbox` 参数，否则会爆出有关于 chrome sandbox 的错误，推测和编写 GUI 的框架有关（Electron？），编辑文件 `~/.config/autostart/clash.desktop`，键入以下内容：

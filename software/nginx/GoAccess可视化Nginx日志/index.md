...
参考：[https://goaccess.io/download](https://goaccess.io/download)
...
增加 `--real-time-html` 参数，实时输出 HTML，GoAccess 会绑定一个端口（默认是 7890，可以通过命令行的 `--port` 或者配置文件修改），启动一个 WebSocket 服务器，实时输出数据。
...
1. [https://example.com/nginx/djhx-site](https://example.com/nginx/djhx-site)
2. [https://example.com/nginx/foo-djhx-site](https://example.com/nginx/foo-djhx-site)
3. [https://example.com/nginx/bar-djhx-site](https://example.com/nginx/bar-djhx-site)

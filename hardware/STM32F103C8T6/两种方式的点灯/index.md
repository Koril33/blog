...
SPL（Standard Peripheral Library）是 ST 公司早些时候开发的框架，为了方便程序员不用再去记忆和操纵那丑陋的寄存器地址而诞生，但是现在 ST 公司已经决定停止开发这款软件，新版本的芯片不再支持它，但大部分的教程还在用这个库，还有很多很多的过去的历史项目都离不开它（就像后端开发里的 Java 8 + Spring Boot 2.x + `CentOS7.x` 组合哈哈哈）。
